...
// Distinct, sorted owner names present in the items (non-empty only).
export function distinctOwners(items) {
  return [...new Set(items.map((i) => i.owner).filter((o) => o && o.trim()))].sort((a, b) =>
    a.localeCompare(b),
  )
}
